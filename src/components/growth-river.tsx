...
export const GrowthRiver = memo(function GrowthRiver({
  ...
  /**
   * The highlighted constraint is the earliest critical stage; if none is
   * critical, the earliest watch stage is surfaced as a heads-up. Stages
   * arrive in funnel order, so the first match is the earliest loss — nothing
   * downstream is blamed first.
   */
   ...
   * meaningful number); otherwise fall back to the stage conversion rate so the
   * callout still carries a real value instead of going blank.
   */
  const bottleneckDelta =
    bottleneckStage &&
    bottleneckStage.benchmark != null &&
    bottleneckStage.conversionRate != null
      ? (bottleneckStage.benchmark - bottleneckStage.conversionRate) * 100
      : null;
          ...
          aria-label={
            bottleneckStage
              ? bottleneckStage.health === "critical"
                ? `Growth river from discovery to renewal, with a confirmed ${bottleneckStage.label.toLowerCase()} bottleneck`
                : `Growth river from discovery to renewal, with ${bottleneckStage.label.toLowerCase()} needing attention`
